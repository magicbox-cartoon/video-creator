...
async function generateVideo() {
  ...
  reader.onloadend = async () => {
    ...
    if (!postData.id) {
      return alert("Error creating video: " + (postData.detail || JSON.stringify(postData)));
    }

    const talkId = postData.id;
    document.getElementById("output").innerHTML = `<p>Processing video (ID: ${talkId})... please wait 📦</p>`;
    ...
    while (status !== "done") {
      ...
      const getResp = await fetch(statusUrl, { headers: { "Authorization": `Bearer ${apiKey}` } });
      const getData = await getResp.json();
      status = getData.status;
      videoUrl = getData.result_url;
    }

    // Step 3: Display final video
